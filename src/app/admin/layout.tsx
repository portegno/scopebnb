    ...
    return (
      <Centered>
        <div className="text-center">
          <h1 className="text-lg font-semibold text-slate-900">Sign in to access the admin</h1>
          <Link
            href="/login"
            className="mt-4 inline-flex h-10 items-center rounded-[4px] bg-slate-900 px-4 text-sm font-semibold text-white hover:bg-slate-800"
          >
            Log in
          </Link>
        </div>
    ...
    <div className="flex min-h-screen bg-slate-50 text-slate-700">
      <aside className="flex w-60 shrink-0 flex-col border-r border-slate-200 bg-white">
        <div className="flex h-16 items-center gap-2 border-b border-slate-200 px-5">
          ...
          <span className="rounded-[4px] bg-slate-900 px-1.5 py-0.5 text-[10px] font-semibold uppercase tracking-wider text-white">
            Admin
          </span>
        </div>
        ...
        <div className="border-t border-slate-200 p-3">
          <p className="truncate px-3 text-xs text-slate-400" title={user?.email ?? undefined}>
            {user?.email}
          </p>
          <button
            onClick={() => signOut()}
            className="mt-1 w-full rounded-[4px] px-3 py-2 text-left text-sm text-slate-500 hover:bg-slate-50 hover:text-slate-900"
          >
            Log out
          </button>
        </div>
